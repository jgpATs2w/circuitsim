CSim = {
	elemDef : {
		...
		I:{
			...
			unit:'amperios',
			symbol:'A',
			label:'Fuente de corriente ideal',
			defValue: 10,
			iniRotation: 0,
			label: true,
			editable: true,
		...
		Ireal:{
			...
			unit:'Amperios',
			symbol:'A',
			label:'Fuente de corriente real',
			defValue: 10,
			iniRotation: 0,
			label: true,
			editable: true,
	...
	_addelementstomatrix : function (){
		...
		for (var i in CSim.elements){
			...
			switch ( elem.type ){
				...
				case "Ireal":
					CSim._addtomatrix(x0, y0, x1, y1, "w", this._matrizcircuito); 
					CSim._addtomatrix(x1, y1, xm0, ym0, 'I', this._matrizcircuito);
					CSim._addtomatrix(xm0, ym0, xm, ym, "w", this._matrizcircuito); 
					CSim._addtomatrix(xm, ym, xm1, ym1, 'R', this._matrizcircuito);
					CSim._addtomatrix(x2, y2, xf, yf, "w", this._matrizcircuito);
					break;
				case "Rvar":
				case "Rrec":
					CSim._addtomatrix(x0, y0, x1, y1, "w", this._matrizcircuito);
					CSim._addtomatrix(x1, y1, x2, y2, 'R', this._matrizcircuito); 
					CSim._addtomatrix(x2, y2, xf, yf, "w", this._matrizcircuito);
					break;
				case "Switch":
					CSim._addtomatrix(x0, y0, xf, yf, '', this._matrizcircuito);
					break;
				case "Fuse":
					CSim._addtomatrix(x0, y0, xf, yf, 'R', this._matrizcircuito);
					break;
				case "Volt":
					CSim._addtomatrix(x0, y0, xf, yf, 'R', this._matrizcircuito);
					break;
				case "Amp":
					CSim._addtomatrix(x0, y0, xf, yf, 'w', this._matrizcircuito);
					break;
				case "G":
					CSim._groundnode = {x: x0, y: y0};	
					break;
				default:
					CSim.onError('unknown type '+elem.type);
			}
	...
	_generarnetlist : function (){
		...
		if (!this._everythingfine) this._say("el circuito está abierto", 'error');
	},
			...
			for (var j=this._ymin*2/this._malla; j<=this._ymax*2/this._malla; j++){	
				for (var i=this._xmin*2/this._malla; i<=this._xmax*2/this._malla; i++){
					...
					if( this._matriznodos[i][j] == "w"){
						...
						if (CSim._elemtypes.indexOf(vtop) != -1 || CSim._elemtypes.indexOf(vleft) != -1){
							this._matriznodos[i][j] = nodetemp;
							temptoreal[nodetemp]=nodetemp;		
							nodetemp ++;			
						}
	...
	_solveMNA : function (netlist){
		...
		if (A.rank() == A.rows()){
			...
			this.onError('A is singular!! A.rank()='+A.rank()+' < A.rows()='+A.rows());
	...
	_keywindow : function(e){
		//console.info(e.which);
			...
			};// test - tecla 'c' para mostrar la matriz de circuito
			if (e.which == 67){
				CSim._mostrarmatrizcircuito();
			};
			// test - tecla 's' para resolver
			if (e.which == 83){
				CSim._solve();
			};
			// test - tecla 's' para resolver
...
CSimElement = function(menuName, x, y){
	...
	this.type = this._menuName.substr(this._menuName.indexOf('_')+1, this._menuName.length);
	
	switch (this.type ){
		case "V":
		case "R":
		case "I":
			var name = this.type+CSim.elemDef[this.type]['counter'];
		...
		case "G":
			...
			break;
		case "Ireal":
		 	var rname = 'R'+CSim.elemDef['R']['counter'];
		 	var iname = 'I'+CSim.elemDef['I']['counter'];
			CSim.elemDef['R']['counter'] ++;
			CSim.elemDef['I']['counter'] ++;
			var name = [rname,iname];
			break;
	}
...
CSimElement.prototype._getDef = function(prop){
	return CSim.elemDef[this.type][prop];
}
...
CSimElement.prototype._loadLabel = function(){
	...
	var str = ( CSim.elemDef[this.type].label ) ? this.name + " = " + this.value + CSim.elemDef[this.type].symbol : "";
...
CSimElement.prototype._getelemcoordinates = function(){
	...
	if (rotation % Math.PI == 0){
		...
		xm = (x0+xf)/2; xm0 = xm - CSim._malla; xm1 = xm + CSim._malla;
		...
		ym = (y0 + yf)/2; ym0 = ym - CSim._malla; ym1 = ym + CSim._malla;
...
CSimEditor = {
	...
	show : function(image){
		...
		if( !CSim.elemDef[CSimEditor.elem.type].editable ) return;
...
CSimCanvas = {
	...
	_mousemove : function (ev){
		...
		if (CSimCanvas._drawing){
			...
			//if( x-CSimCanvas._x0 < CSim._malla && y-CSimCanvas._y0 < CSim._malla ) return false;
